...
describe('writeAiOutputs', () => {
  it('writes structured AI files from the content graph', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-ai-'))
    ...
    const pages = JSON.parse(await readFile(join(cwd, '.docfuse/dist/ai/pages.json'), 'utf8'))
    const examples = JSON.parse(await readFile(join(cwd, '.docfuse/dist/ai/code-examples.json'), 'utf8'))
    const index = await readFile(join(cwd, '.docfuse/dist/ai/index.md'), 'utf8')
    const llms = await readFile(join(cwd, '.docfuse/dist/llms.txt'), 'utf8')
    const manifest = JSON.parse(await readFile(join(cwd, '.docfuse/dist/ai/manifest.json'), 'utf8'))
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/llms-full.txt'), 'utf8')).not.toContain(
    ...
    expect(await readFile(join(cwd, '.docfuse/dist', shardPath), 'utf8')).toContain('"content":"# 首页"')
    ...
    const allVersionsConfig = {
      ...defaultConfig,
      outputDir: '.docfuse/all-versions',
  ...
  it('partitions large Unicode content into bounded shards and keeps llms-full as a manifest pointer', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-ai-bounded-'))
    await mkdir(join(cwd, '.docfuse/dist'), { recursive: true })
    const body = `# 大页面\n\n${'中文内容\\n'.repeat(400)}`
    const graph = createMockGraph({
      pages: [
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/llms-full.txt'), 'utf8')).toContain('/ai/manifest.json')
    const shardPaths = manifest.partitions[0].shards.map((shard: { path: string }) => shard.path)
    const records = []
    for (const shardPath of shardPaths) {
      const absolutePath = join(cwd, '.docfuse/dist', shardPath.replace(/^\//, ''))
  ...
  it('can fail atomically instead of degrading llms-full when configured', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-ai-error-'))
    await mkdir(join(cwd, '.docfuse/dist'), { recursive: true })
    const graph = createMockGraph({
      pages: [
        createMockPage({
