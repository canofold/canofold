...
describe('redirects', () => {
  it('writes redirect pages and a deployment manifest', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-redirects-'))
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/old/index.html'), 'utf8')).toContain(
      'location.replace("/guide/")'
    )
    expect(JSON.parse(await readFile(join(cwd, '.docfuse/dist/redirects.json'), 'utf8'))).toEqual({
      '/old/': '/guide/'
    })
  })
  ...
  it('writes an absolute canonical URL when siteUrl is configured', async () => {
    ...
    const html = await readFile(join(cwd, '.docfuse/dist/old/index.html'), 'utf8')
