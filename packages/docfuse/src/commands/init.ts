...
export async function promptForLocales(
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ABORT_ERR') {
      throw new Error('Docfuse initialization cancelled')
    }
    throw error
  } finally {
    readline.close()
...
function assertOptionsMatchConfig(options: InitOptions, config: DocfuseConfig) {
...
export async function runInit(options: InitOptions, runtime: InitRuntime = {}) {
    ...
    if (!localePrompt) {
      ...
      throw new Error(
        `Found ${existingDocuments.length} existing Markdown file(s) in "${docsDir}" (e.g. "${examplePath}"), ` +
          ...
          `Specify the locale explicitly: docfuse init --locale <locale> (e.g. --locale zh)`
      )
    }
  ...
  if (!plan.createFiles.length) {
    ...
    logInfo(`Docfuse is already initialized in ${root}`)
    return
  }
  ...
  if (plan.mode === 'adopt') {
    ...
    logInfo('Next: docfuse check')
  } else if (plan.mode === 'configured') {
    logInfo(`Created missing documentation files in ${root}`)
    logInfo('Next: docfuse dev')
  } else {
    logInfo(`Created Docfuse docs in ${root}`)
    logInfo('Next: docfuse dev')
  }
  logInfo('Add .docfuse/ to .gitignore if this directory is versioned.')
}
