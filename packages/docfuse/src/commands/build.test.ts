...
describe('runBuild', () => {
  it('compiles uppercase MDX extensions with MDX semantics', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-extension-case-'))
    ...
    const html = await readFile(join(cwd, '.docfuse/dist/Guide/index.html'), 'utf8')
    expect(html).toContain('<p data-df-element="paragraph">MDX value: 2</p>')
  })

  it('builds deployable static output', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-'))
    ...
    await access(join(cwd, '.docfuse/dist/index.html'))
    await expect(access(join(cwd, '.docfuse/dist/zh/index.html'))).rejects.toMatchObject({ code: 'ENOENT' })
    await access(join(cwd, '.docfuse/dist/en/index.html'))
    await access(join(cwd, '.docfuse/dist/search/zh.json'))
    await access(join(cwd, '.docfuse/dist/llms.txt'))
    await access(join(cwd, '.docfuse/dist/ai/pages.json'))
    await access(join(cwd, '.docfuse/dist/ai/manifest.json'))

    const html = await readFile(join(cwd, '.docfuse/dist/index.html'), 'utf8')
  ...
  it('reuses a valid cross-process cache and safely recovers from output and manifest corruption', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-cache-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    ...
    await writeFile(join(cwd, '.docfuse/dist/index.html'), 'tampered')
    expect(await runBuild({ cwd })).toMatchObject({ mode: 'clean', reason: 'invalid-output' })
    expect(await readFile(join(cwd, '.docfuse/dist/index.html'), 'utf8')).toContain('<!doctype html>')

    await writeFile(join(cwd, '.docfuse/cache/build-manifest.json'), '{broken')
  ...
  it('invalidates cached analysis, rendering, and generated artifacts when an extension changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-extension-'))
    await mkdir(join(cwd, 'docs'), { recursive: true })
    ...
    expect(await runBuild({ cwd })).toMatchObject({ mode: 'clean', reason: 'missing-manifest' })
    expect(await readFile(join(cwd, '.docfuse/dist/index.html'), 'utf8')).toContain('alpha')
    expect(await readFile(join(cwd, '.docfuse/dist/index.md'), 'utf8')).toContain('alpha')
    expect(await readFile(join(cwd, '.docfuse/dist/index.md'), 'utf8')).not.toContain('{{release}}')
    expect(
      JSON.parse(await readFile(join(cwd, '.docfuse/dist/extensions/release/metadata.json'), 'utf8'))
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/index.html'), 'utf8')).toContain('beta')
    expect(await readFile(join(cwd, '.docfuse/dist/index.md'), 'utf8')).toContain('beta')
    expect(
      JSON.parse(await readFile(join(cwd, '.docfuse/dist/extensions/release/metadata.json'), 'utf8'))
  ...
  it('invalidates only MDX pages that import a changed project-local component', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-local-component-'))
    ...
    await runBuild({ cwd })
    expect(await readFile(join(cwd, '.docfuse/dist/guide/status/index.html'), 'utf8')).toContain('Alpha')
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/guide/status/index.html'), 'utf8')).toContain('Beta')
  })

  it('invalidates generated extension artifacts when a bundled package dependency changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-extension-dependency-'))
    await mkdir(join(cwd, 'docs'), { recursive: true })
    await mkdir(join(cwd, 'extensions'), { recursive: true })
    ...
    await runBuild({ cwd })
    expect(
      JSON.parse(await readFile(join(cwd, '.docfuse/dist/extensions/report/metadata.json'), 'utf8'))
    ).toEqual({ label: 'alpha' })
    expect(await runBuild({ cwd })).toMatchObject({ mode: 'cached' })
  ...
  it('handles multi-page changes and deletions with output identical to a clean build', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-equivalence-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '# Home')
    ...
    expect(
      await directorySnapshot(join(cwd, '.docfuse/dist'), join(cwd, '.docfuse/dist'), new Set(['pagefind']))
    ...
    await expect(access(join(cwd, '.docfuse/dist/second/index.html'))).rejects.toMatchObject({
      code: 'ENOENT'
    })
  ...
  it('rejects output directories that overlap source documents without deleting them', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-safety-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    const source = join(cwd, 'docs/zh/index.mdx')
    await writeFile(source, '# Keep me')
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { outputDir: 'docs' }`)
  ...
  it('rejects source/output overlap hidden behind a symbolic link without deleting data', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-symlink-safety-'))
  ...
  it('rejects an output directory that escapes through a symbolic-link ancestor', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-output-link-'))
    const outside = await mkdtemp(join(tmpdir(), 'docfuse-build-output-outside-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '# Home')
    await symlink(outside, join(cwd, '.docfuse'))
  ...
  it('preserves the last successful output when a later build fails', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-atomic-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '# Home')
    const source = join(cwd, 'docs/zh/guide.mdx')
    await writeFile(source, '# Stable output')
    await runBuild({ cwd })
    const output = join(cwd, '.docfuse/dist/guide/index.html')
    const previous = await readFile(output, 'utf8')

    await writeFile(source, `import Broken from 'not-a-docfuse-component'\n\n# Broken output`)
    await expect(runBuild({ cwd })).rejects.toThrow()
  ...
  it('recovers the previous output after an interrupted directory swap', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-swap-recovery-'))
    await mkdir(join(cwd, 'docs'), { recursive: true })
    await writeFile(join(cwd, 'docs/index.md'), '# Stable output')
    await runBuild({ cwd })

    const outputRoot = join(cwd, '.docfuse/dist')
    const backupRoot = join(cwd, '.docfuse/.dist.backup')
  ...
  it('rejects public assets that use the build-owned benchmark path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-benchmark-collision-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
  ...
  it('rejects page routes inside runtime-owned asset directories before rendering', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-build-runtime-route-collision-'))
  ...
  it('rebuilds one changed page when its navigation contract is stable', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '---\ntitle: 首页\n---\n# 首页')
    await writeFile(join(cwd, 'docs/zh/guide.md'), '---\ntitle: 指南\n---\n# 指南\n\n旧内容')

    await runBuild({ cwd })
    const sharedCss = join(cwd, '.docfuse/dist/assets/docfuse.css')
  ...
  it('falls back to clean builds when the site-level math resource fact changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-math-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '# Home')
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/assets/docfuse.css'), 'utf8')).toContain('KaTeX_Main')
    expect(
      (await readdir(join(cwd, '.docfuse/dist/assets/fonts'))).filter((file) => file.endsWith('.woff2'))
    ).toHaveLength(20)
    expect(await readFile(join(cwd, '.docfuse/dist/guide/index.html'), 'utf8')).toContain('$E = mc^2$')
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/assets/docfuse.css'), 'utf8')).not.toContain('KaTeX_Main')
    await expect(access(join(cwd, '.docfuse/dist/assets/fonts'))).rejects.toMatchObject({ code: 'ENOENT' })
  })

  it('rebuilds shared assets when the site gains or loses its first playground', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-playground-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '# Home')
    const guidePath = join(cwd, 'docs/zh/guide.md')
    await writeFile(guidePath, '# Guide\n\nPlain content')
    ...
    await runBuild({ cwd })
    const playgroundBundle = join(cwd, '.docfuse/dist/assets/docfuse-playground/index.js')
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/guide/index.html'), 'utf8')).toContain(
      'data-docfuse-playground-client-url="/assets/docfuse-playground/index.js"'
    )
  ...
  it('removes page assets that are no longer referenced after an incremental rebuild', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-stale-asset-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '# Home')
    const guidePath = join(cwd, 'docs/zh/guide.md')
    await writeFile(guidePath, '# Guide\n\n![Old](./old.png)')
    ...
    await runBuild({ cwd })
    const generatedAsset = join(cwd, '.docfuse/dist/guide/old.png')
  ...
  it('removes plugin assets that are no longer required after an incremental rebuild', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-plugin-assets-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    ...
    await runBuild({ cwd })
    const pluginRoot = join(cwd, '.docfuse/dist/assets/docfuse-plugins')
  ...
  it('preserves a public asset when a page stops sharing ownership of its output path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-shared-asset-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    ...
    await runBuild({ cwd })
    const generatedAsset = join(cwd, '.docfuse/dist/guide/shared.png')
    await access(generatedAsset)
    await writeFile(guidePath, '# Guide\n\nNo image now.')
  ...
  it('rerenders affected pages when navigation metadata changes', async () => {
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/guide/index.html'), 'utf8')).toContain('新指南')
  })

  it('rerenders affected pages when a directory group label changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-group-'))
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/guide/start/index.html'), 'utf8')).toContain('Tutorials')
  })

  it('adds required Islands when an incremental page gains core interactive blocks', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-islands-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(join(cwd, 'docs/zh/index.md'), '---\ntitle: 首页\n---\n# 首页')
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { markdown: { html: 'trusted' } }`)
    const guidePath = join(cwd, 'docs/zh/guide.md')
    await writeFile(guidePath, '---\ntitle: 指南\n---\n# 指南\n\n普通正文')
    ...
    expect(second.incremental).toBe(true)
    await access(join(cwd, '.docfuse/dist/assets/docfuse-markdown/index.js'))
    const html = await readFile(join(cwd, '.docfuse/dist/guide/index.html'), 'utf8')
    expect(html).toContain('data-df-island="gallery"')
    expect(html).toContain('data-df-island="table"')
    ...
    await expect(access(join(cwd, '.docfuse/dist/assets/docfuse-markdown'))).rejects.toMatchObject({
      code: 'ENOENT'
    })
  ...
  it('keeps the Markdown client when only an unchanged page contains a GFM table', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-incremental-gfm-table-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    await writeFile(
      join(cwd, 'docs/zh/index.md'),
      ['# Home', '', '| Name | Value |', '| --- | --- |', '| Docfuse | Fast |'].join('\n')
    )
    const guidePath = join(cwd, 'docs/zh/guide.md')
    await writeFile(guidePath, '# Guide\n\nBefore')

    await runBuild({ cwd })
    const markdownClient = join(cwd, '.docfuse/dist/assets/docfuse-markdown/index.js')
    ...
    await access(markdownClient)
    expect(await readFile(join(cwd, '.docfuse/dist/index.html'), 'utf8')).toContain('data-df-island="table"')
  })

  it('tracks versioned pages independently when versions share one docsDir', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-shared-version-docs-'))
    await mkdir(join(cwd, 'docs'), { recursive: true })
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/index.html'), 'utf8')).toContain('Beta')
    expect(await readFile(join(cwd, '.docfuse/dist/v1/index.html'), 'utf8')).toContain('Beta')
  })

  it('builds versioned snapshots, isolated search indexes, and redirects', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-enterprise-'))
    await mkdir(join(cwd, 'docs/zh/guide'), { recursive: true })
    ...
    const current = await readFile(join(cwd, '.docfuse/dist/guide/start/index.html'), 'utf8')
    const historical = await readFile(join(cwd, '.docfuse/dist/v1/guide/start/index.html'), 'utf8')
    ...
    await access(join(cwd, '.docfuse/dist/search/zh.json'))
    await access(join(cwd, '.docfuse/dist/getting-started/index.html'))
    await access(join(cwd, '.docfuse/dist/redirects.json'))
  })
})
