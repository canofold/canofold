...
function createGuides(
  ...
  const nginxRoot = quoteNginxValue(`/var/www/docfuse/${outputPath.replace(/^\/+/, '')}`)
  ...
  const buildCommand = {
    npm: 'npm exec -- docfuse build',
    pnpm: 'pnpm exec docfuse build',
    yarn: 'yarn exec docfuse build'
  }[packageManager]
...
export async function runDeploy({ cwd }: { cwd: string }) {
  ...
  if (!(await pathExists(join(outputRoot, 'index.html')))) {
    throw new Error(`Build output not found at ${config.outputDir}. Run docfuse build before docfuse deploy.`)
  }

  const deployDir = join(cwd, '.docfuse/deploy')
