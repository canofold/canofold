...
describe('runClean', () => {
  it('removes generated output and persistent build state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-clean-'))
    ...
    await mkdir(join(cwd, '.docfuse/tmp/stale-config'), { recursive: true })
    await writeFile(join(cwd, '.docfuse/tmp/stale-config/config.mjs'), 'export default {}')

    await runClean({ cwd })

    await expect(access(join(cwd, '.docfuse/dist'))).rejects.toMatchObject({ code: 'ENOENT' })
    await expect(access(join(cwd, '.docfuse/cache/build-manifest.json'))).rejects.toMatchObject({
      code: 'ENOENT'
    })
    await expect(access(join(cwd, '.docfuse/cache'))).rejects.toMatchObject({ code: 'ENOENT' })
    await expect(access(join(cwd, '.docfuse/tmp'))).rejects.toMatchObject({ code: 'ENOENT' })
