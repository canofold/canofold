...
import { docfuseVersion } from '../index'
...
describe('promptForLocales', () => {
  it('reports terminal cancellation and always closes the readline interface', async () => {
    ...
    await expect(promptForLocales(readline)).rejects.toThrow('Docfuse initialization cancelled')
...
describe('runInit', () => {
  it('creates docs scaffold in current directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-init-'))
    await runInit({ cwd })

    const zhIndex = await readFile(join(cwd, 'docs/index.md'), 'utf8')
    const config = await readFile(join(cwd, 'docfuse.config.ts'), 'utf8')
    const envDts = await readFile(join(cwd, 'docs/docfuse-env.d.ts'), 'utf8')

    expect(zhIndex).toContain('# 欢迎使用 Docfuse')
    expect(config).toContain("import { defineConfig } from 'docfuse'")
    expect(config).toContain('export default defineConfig({')
    expect(config).toContain("title: 'Docfuse'")
    expect(config).toContain(`requiredVersion: '^${docfuseVersion}'`)
    expect(config).toContain("locales: ['zh']")
    expect(envDts).toContain("declare module 'react/jsx-runtime'")
    expect(envDts).not.toContain("declare module 'docfuse/components'")
  ...
  it('creates locale directories only for non-default languages', async () => {
    ...
    await expect(readFile(join(cwd, 'docs/index.md'), 'utf8')).resolves.toContain('# 欢迎使用 Docfuse')
    ...
    expect(neutralIndex).toContain('# Docfuse')
  ...
  it.each([
    ['en-US', '# Welcome to Docfuse', '# Guide'],
    ['zh-CN', '# 欢迎使用 Docfuse', '# 指南']
  ...
  it('adopts existing documents without modifying or supplementing their content', async () => {
    ...
    await expect(readFile(join(cwd, 'docfuse.config.ts'), 'utf8')).resolves.toContain("defaultLocale: 'en'")
    await expect(readFile(join(cwd, 'docs/docfuse-env.d.ts'), 'utf8')).resolves.toContain(
  ...
  it('adopts existing documents with uppercase Markdown extensions', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-init-extension-case-'))
    await mkdir(join(cwd, 'docs'), { recursive: true })
  ...
  it('reports whether it adopted content and what to do next', async () => {
    ...
    try {
      ...
      expect(output).toContain('Next: docfuse check')
    } finally {
      log.mockRestore()
    }
  })
  ...
  it('requires an explicit locale for existing documents in a non-interactive environment', async () => {
    ...
    await expect(access(join(cwd, 'docs/docfuse-env.d.ts'))).rejects.toMatchObject({
      code: 'ENOENT'
    })
  })
  ...
  it('is a successful no-op when the project is already initialized', async () => {
    ...
    const before = await Promise.all([
      readFile(join(cwd, 'docfuse.config.ts'), 'utf8'),
      readFile(join(cwd, 'docs/index.md'), 'utf8'),
      readFile(join(cwd, 'docs/guide.md'), 'utf8'),
      readFile(join(cwd, 'docs/docfuse-env.d.ts'), 'utf8')
    ])
...
`
    await writeFile(join(cwd, 'docfuse.config.ts'), config)

    await runInit({ cwd })

    await expect(readFile(join(cwd, 'docfuse.config.ts'), 'utf8')).resolves.toBe(config)
    await expect(readFile(join(cwd, 'handbook/index.md'), 'utf8')).resolves.toContain('# Welcome to Docfuse')
    await expect(readFile(join(cwd, 'handbook/zh/index.md'), 'utf8')).resolves.toContain('# 欢迎使用 Docfuse')
    await expect(readFile(join(cwd, 'handbook/docfuse-env.d.ts'), 'utf8')).resolves.toContain(
      "declare module 'react/jsx-runtime'"
    )
  })
...
`
  ...
  it('uses a custom project-relative docs directory and rejects absolute paths', async () => {
    ...
    await expect(readFile(join(cwd, 'docfuse.config.ts'), 'utf8')).resolves.toContain('docsDir: "handbook"')
    await expect(access(join(cwd, 'docs'))).rejects.toMatchObject({ code: 'ENOENT' })

    const another = await mkdtemp(join(tmpdir(), 'docfuse-init-'))
    await expect(runInit({ cwd: another, docsDir: join(another, 'handbook') })).rejects.toThrow(
      'docsDir must be a relative path'
    )
    await expect(access(join(another, 'docfuse.config.ts'))).rejects.toMatchObject({ code: 'ENOENT' })
  })

  it('preflights blocking directories before creating any files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-init-'))
    await mkdir(join(cwd, 'docs/docfuse-env.d.ts'), { recursive: true })

    await expect(runInit({ cwd })).rejects.toThrow(
      'Cannot create docs/docfuse-env.d.ts because a directory exists at that path'
    )
    await expect(access(join(cwd, 'docfuse.config.ts'))).rejects.toMatchObject({ code: 'ENOENT' })
    await expect(access(join(cwd, 'docs/index.md'))).rejects.toMatchObject({ code: 'ENOENT' })
  })
