...
export const noFlashScript =
  "try{var t=localStorage.getItem('docfuse-theme');if(t==='dark'||(!t&&window.matchMedia('(prefers-color-scheme:dark)').matches)){document.documentElement.classList.add('dark')}}catch(e){}"
...
/* v8 ignore next */
export const shellScript = `window.__docfuseBootstrapShell = () => {
  window.__docfuseShellDispose?.()
  ...
  const homeHeader = q('.df-header-home')
  function syncHomeHeader() {
    homeHeader?.classList.toggle('df-header-scrolled', window.scrollY > 8)
  }
  syncHomeHeader()
  on(window, 'scroll', syncHomeHeader, { passive: true })

  function disposePage() {
    window.__docfuseSearchDispose?.()
    window.__docfuseMarkdownDispose?.()
    window.__docfuseOutlineDispose?.()
    window.__docfuseShellDispose?.()
  }

  async function bootstrapPage() {
    window.__docfuseBootstrapShell?.()
    window.__docfuseBootstrapOutline?.()
    window.__docfuseBootstrapSearch?.()
    await window.__docfuseBootstrapMarkdown?.()
    const pageRoot = q('[data-docfuse-page-root]')
    const pageModule = pageRoot?.getAttribute('data-docfuse-playground-client-url')
    if (pageModule) {
      const loadModule = window.__docfuseLoadPageModule || ((url) => import(url))
      await loadModule(pageModule)
      window.__docfuseBootstrapPlayground?.()
    }
  }
  ...
  function captureUpdateState(pageRoot) {
    ...
    return {
      ...
      tabs: qa('[data-df-behavior="tabs"]', pageRoot).map((item) => ({
        id: item.getAttribute('data-df-tabs-id'),
        value: q('[role="tab"][data-df-tab][aria-selected="true"]', item)?.getAttribute('data-df-tab') || null
      }))
    }
  }
  ...
  function restoreUpdateTabs(pageRoot, state) {
    state.tabs.forEach((saved) => {
      ...
      const tabs = qa('[data-df-behavior="tabs"]', pageRoot).find(
        (item) => item.getAttribute('data-df-tabs-id') === saved.id
      )
      const trigger = tabs && qa('[role="tab"][data-df-tab]', tabs).find(
        (item) => item.getAttribute('data-df-tab') === saved.value
      )
      trigger?.click()
    })
  }
  ...
  function syncPageHead(nextDocument) {
    ...
    const currentNodes = qa('[data-docfuse-page-head]', d.head)
    const nextNodes = qa('[data-docfuse-page-head]', nextDocument.head)
  ...
  function syncSidebarState(currentSidebar, nextSidebar) {
    const currentLinks = qa('[data-docfuse-sidebar-link]', currentSidebar)
    const nextLinks = qa('[data-docfuse-sidebar-link]', nextSidebar)
  ...
  async function applyPageDocument(nextDocument, options = {}) {
    const currentRoot = q('[data-docfuse-page-root]')
    const nextRoot = q('[data-docfuse-page-root]', nextDocument)
    if (!currentRoot || !nextRoot) return false
    const updateState = options.mode === 'update' ? captureUpdateState(currentRoot) : null
    const sidebarScrollTop = options.sidebarScrollTop ?? q('[data-docfuse-sidebar]', currentRoot)?.scrollTop ?? 0
    disposePage()
    const imported = d.importNode(nextRoot, true)
    if (options.mode === 'navigation') {
      const currentSidebar = q('[data-docfuse-sidebar]', currentRoot)
      const nextSidebar = q('[data-docfuse-sidebar]', imported)
      if (
        currentSidebar &&
        nextSidebar &&
        currentSidebar.getAttribute('data-docfuse-sidebar-key') &&
        currentSidebar.getAttribute('data-docfuse-sidebar-key') ===
          nextSidebar.getAttribute('data-docfuse-sidebar-key') &&
    ...
    if (updateState) {
      ...
      if (options.focus) q('#docfuse-main', imported)?.focus({ preventScroll: true })
      if (options.hash) {
  ...
  window.__docfuseApplyPageDocument = applyPageDocument

  function currentNavigationState() {
    return {
      scrollY: window.scrollY,
      sidebarScrollTop: q('[data-docfuse-sidebar]')?.scrollTop || 0
    }
  }

  function historyStateWithNavigation(state) {
    const current = history.state && typeof history.state === 'object' ? history.state : {}
    return { ...current, __docfuse: state }
  }
  ...
  async function navigatePage(url, { push = true, state } = {}) {
    ...
    if (!q('[data-docfuse-page-root]', nextDocument)) return false
  ...
  qa('[data-docfuse-theme-toggle]').forEach((button) => {
    button.setAttribute('aria-pressed', String(root.classList.contains('dark')))
    on(button, 'click', () => {
      const on = root.classList.toggle('dark')
      button.setAttribute('aria-pressed', String(on))
      try { localStorage.setItem('docfuse-theme', on ? 'dark' : 'light') } catch {}
    })
  })

  const sourceSheet = q('[data-docfuse-source-sheet]')
  let sourceLastFocus
  ...
  function setSource(value, invoker) {
    ...
    if (value) {
      ...
      root.classList.add('df-source-open')
      requestAnimationFrame(() => (focusable(sourceSheet)[0] || q('.df-source-panel', sourceSheet))?.focus())
    } else {
      const wasOpen = !sourceSheet.hidden
      sourceSheet.hidden = true
      root.classList.remove('df-source-open')
      if (wasOpen && sourceLastFocus && typeof sourceLastFocus.focus === 'function') sourceLastFocus.focus()
      sourceLastFocus = undefined
    }
  }
  qa('[data-docfuse-source-open]').forEach((button) => on(button, 'click', () => {
    setSource(true, button)
  }))
  qa('[data-docfuse-source-close]').forEach((button) => on(button, 'click', () => {
    setSource(false)
  }))
  qa('[data-docfuse-source-copy]').forEach((button) => on(button, 'click', () => {
    const source = q('[data-docfuse-source-text]')
    const success = button.getAttribute('data-docfuse-action-success') || 'Markdown source copied'
    if (source) copyText(source.value).then(() => markAction(button, success)).catch(() => {})
  }))

  qa('[data-docfuse-playground-toggle]').forEach((button) => on(button, 'click', () => {
    const playground = button.closest('[data-docfuse-playground]')
  ...
  qa('[data-docfuse-playground-resizer]').forEach((resizer) => {
    const playground = resizer.closest('[data-docfuse-playground]')
    if (!playground) return
    ...
    const setSplit = (value) => {
      ...
      playground.style.setProperty('--df-playground-source-width', next + '%')
  ...
  const bar = q('[data-docfuse-progress]')
  function progress() {
    if (!bar) return
  ...
  const sidebar = q('[data-docfuse-sidebar]')
  const backdrop = q('[data-docfuse-sidebar-backdrop]')
  const open = q('[data-docfuse-sidebar-open]')
  function setSidebar(value) {
    ...
    if (open) {
      ...
      open.setAttribute(
        'aria-label',
        value ? open.dataset.docfuseSidebarCloseLabel : open.dataset.docfuseSidebarOpenLabel
      )
    }
  }
  if (open) {
  ...
  qa('[data-docfuse-sidebar-link]').forEach((link) => on(link, 'click', () => setSidebar(false)))

  on(d, 'click', (event) => {
    qa('[data-docfuse-menu][open]').forEach((menu) => {
    ...
    if (!anchor || anchor.hasAttribute('download') || anchor.hasAttribute('data-docfuse-no-navigation')) return
  ...
  on(window, 'popstate', (event) => {
    const state = event.state?.__docfuse || { scrollY: 0, sidebarScrollTop: 0 }
  ...
  on(d, 'keydown', (event) => {
    ...
    setSidebar(false)
    qa('[data-docfuse-menu][open]').forEach((menu) => menu.removeAttribute('open'))
    setSource(false)
  })
  window.__docfuseShellDispose = () => {
    controller.abort()
...
/* v8 ignore next */
export const outlineScript = `window.__docfuseBootstrapOutline = () => {
  window.__docfuseOutlineDispose?.()
  const controller = new AbortController()
  const signal = controller.signal
  const links = Array.from(document.querySelectorAll('[data-docfuse-outline-link]'))
  ...
  function setActive(link) {
    ...
    const panel = link.closest('[data-docfuse-outline]')
    if (!panel) return
  ...
  function update() {
    ...
    const header = document.querySelector('.df-header')?.getBoundingClientRect().height || ${DEFAULT_HEADER_HEIGHT_PX}
