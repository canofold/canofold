...
import type { DocfuseConfig, ThemeBaseColor } from '../config/types'
...
const accentColors: Record<string, { light: string; dark: string }> = {
  docfuse: { light: DEFAULT_SEMANTIC_COLORS.light.blue, dark: DEFAULT_SEMANTIC_COLORS.dark.blue },
...
const basePalettes = {
  ...
  neutral: {
    light: {
      ...
      text: 'color-mix(in oklab, var(--df-ink) 88%, var(--df-ink-secondary))',
...
function resolveTokens(config: DocfuseConfig, mode: ThemeMode): PaletteTokenValues {
  ...
  const usesPaletteAccent = config.theme.accentColor === 'docfuse'
  ...
  return {
    ...
    primarySoft:
      usesPaletteAccent && palette.primarySoft
        ? palette.primarySoft
        : `color-mix(in oklab, ${primary} 12%, var(--df-surface))`,
    primaryForeground:
      usesPaletteAccent && palette.primaryForeground
        ? palette.primaryForeground
        : `color-mix(in oklab, ${primary} 76%, var(--df-ink))`,
    primaryDeep:
      usesPaletteAccent && palette.primaryDeep
        ? palette.primaryDeep
        : `color-mix(in oklab, ${primary} 84%, var(--df-ink))`,
...
function resolveDocfuseTheme(config: DocfuseConfig): ResolvedMarkdownTheme {
...
function paletteCss(mode: ThemeMode, theme: ResolvedMarkdownTheme) {
  ...
  --df-accent-50: color-mix(in oklab, ${primary} 8%, var(--df-surface));
  --df-accent-100: color-mix(in oklab, ${primary} 16%, var(--df-surface));
  --df-accent-200: color-mix(in oklab, ${primary} 28%, var(--df-surface));
  --df-accent-300: color-mix(in oklab, ${primary} 48%, var(--df-surface));
  --df-accent-400: color-mix(in oklab, ${primary} 78%, var(--df-surface));
  --df-accent-600: color-mix(in oklab, ${primary} 88%, var(--df-ink));
  --df-accent-700: color-mix(in oklab, ${primary} 76%, var(--df-ink));
  --df-accent-800: color-mix(in oklab, ${primary} 62%, var(--df-ink));
  --df-accent-900: color-mix(in oklab, ${primary} 48%, var(--df-ink));`
}

/**
 * Emit Docfuse design-token overrides for the configured palette and radius.
 * Appended after the compiled stylesheet so project-level configuration wins.
 */
export function buildThemeVariables(config: DocfuseConfig) {
  const theme = resolveDocfuseTheme(config)
...
${paletteCss('light', theme)}
  --df-sidebar-width: ${sidebarWidth};
  --df-outline-width: ${outlineWidth};
}`
