...
describe('createLayoutModel', () => {
  ...
  it('accepts both cleanup functions and dispose objects from plugin clients', () => {
    ...
    expect(model.markdownClientScript).toContain('window.__docfuseEnhanceMarkdown')
    expect(model.markdownClientScript).toContain('window.__docfuseMarkdownReady = ready')
