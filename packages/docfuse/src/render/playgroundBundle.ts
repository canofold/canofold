...
export async function buildPlaygroundBundle(cwd: string, outputRoot: string, config: DocfuseConfig) {
  const target = join(outputRoot, 'assets/docfuse-playground')
  ...
  const imports = descriptors
    .map(
      (descriptor, index) =>
        `import { ${descriptor.exportName} as docfusePlugin${index} } from ${jsonForJavaScript(descriptor.module)}`
    )
    .join('\n')
  const plugins = descriptors
    .map((descriptor, index) =>
      descriptor.options === undefined
        ? `docfusePlugin${index}()`
        : `docfusePlugin${index}(${jsonForJavaScript(descriptor.options)})`
    )
    .join(',\n  ')
  const markdown = {
...
window.__docfuseBootstrapPlayground = () => mountPlaygrounds(options)
window.__docfuseBootstrapPlayground()`
  ...
  await build({
    ...
    stdin: {
      ...
      loader: 'js',
      resolveDir: cwd,
      sourcefile: 'docfuse-playground-entry.js'
    },
    bundle: true,
    format: 'esm',
    platform: 'browser',
