...
import { analyzeMdxModuleBoundary } from '@docfuse/markdown/server/analyze'
...
describe('local MDX components', () => {
  it('does not expose component runtimes through process-global state', () => {
    expect((globalThis as Record<PropertyKey, unknown>)[Symbol.for('docfuse.local-component-runtime')]).toBe(
      undefined
    )
  })

  it('loads trusted local TSX components', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-local-component-'))
...
`
  ...
  it('reports missing local exports before React rendering', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-local-missing-export-'))
  ...
  it('renders hook-based local components with the host React instance', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-local-hooks-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    const pagePath = join(cwd, 'docs/zh/index.mdx')
    await writeFile(
...
function Gallery({ children }) {
  return <div data-df-component="gallery">{children}</div>
}
...
`
  ...
  it('does not expose retired package component runtimes to local components', async () => {
    ...
    await writeFile(
      join(cwd, 'docs/zh/LocalGallery.tsx'),
      `import { MarkdownGallery } from '@docfuse/markdown'
...
`
    ...
    await expect(
      loadLocalComponentBundle(importsFor("import { LocalGallery } from './LocalGallery'"), pagePath, cwd)
    ).rejects.toThrow('External import is not allowed in local docs components: @docfuse/markdown')
  })
...
`
  ...
  it('rejects relative imports that escape the project root', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-local-component-'))
    const outside = await mkdtemp(join(tmpdir(), 'docfuse-outside-'))
    await mkdir(join(cwd, 'docs/zh'), { recursive: true })
    const pagePath = join(cwd, 'docs/zh/index.mdx')
    await writeFile(
