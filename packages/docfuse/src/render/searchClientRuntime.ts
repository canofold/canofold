...
export function bootstrapSearch() {
  ...
  let disposed = false
  const modal = document.querySelector<HTMLElement>('[data-docfuse-search]')
  ...
  if (modal) {
    ...
    const defaults = activeModal.querySelector<HTMLElement>('[data-docfuse-search-default]')
    const results = activeModal.querySelector<HTMLElement>('[data-docfuse-search-results]')
    const openers = document.querySelectorAll<HTMLElement>('[data-docfuse-search-open]')
    const closers = activeModal.querySelectorAll<HTMLElement>('[data-docfuse-search-close]')
    const actions = activeModal.querySelectorAll<HTMLElement>('[data-docfuse-search-action]')
    ...
    function openModal() {
      ...
      document.documentElement.classList.add('df-search-open')
    ...
    function closeModal() {
      ...
      document.documentElement.classList.remove('df-search-open')
    ...
    actions.forEach((button) =>
      on(button, 'click', () => {
        const action = button.getAttribute('data-docfuse-search-action')
        closeModal()
        if (action === 'source') {
          document.querySelector<HTMLElement>('[data-docfuse-source-open]')?.click()
        }
        if (action === 'theme') {
          document.querySelector<HTMLElement>('[data-docfuse-theme-toggle]')?.click()
        }
      })
    )
          ...
          try {
            ...
            results.innerHTML = matches.length
              ? matches
                  .map(
                    (document) =>
                      '<a class="df-search-result" href="' +
                      escapeHtml(document.routePath) +
                      '"><span class="df-search-result-copy"><strong>' +
                      highlightSearchText(document.title, query) +
                      '</strong><small>' +
                      highlightSearchText(document.description || document.routePath, query) +
                      '</small></span><span class="df-search-result-arrow" aria-hidden="true">→</span></a>'
                  )
                  .join('')
              : `<p class="df-search-empty">${escapeHtml(emptyLabel)}</p>`
          } catch {
            if (disposed || input.value.trim() !== query) return
            results.innerHTML = `<p class="df-search-empty">${escapeHtml(errorLabel)}</p>`
          }
        }, 150)
      })
    }
...
const runtime = window as SearchWindow
runtime.__docfuseBootstrapSearch = bootstrapSearch
bootstrapSearch()
