...
beforeAll(async () => {
  outputRoot = await mkdtemp(join(tmpdir(), 'docfuse-search-bundle-'))
  ...
  searchBundle = await readFile(join(outputRoot, 'assets/docfuse-search.js'), 'utf8')
})
