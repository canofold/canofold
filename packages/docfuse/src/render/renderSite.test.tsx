...
describe('renderSite', () => {
  it('fails explicitly when required KaTeX fonts cannot be copied', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-missing-math-fonts-'))
  ...
  it('omits the Markdown article when a landing page has no authored body', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-empty-landing-'))
  ...
  it('renders a playground from one Markdown source without the document footer or outline', async () => {
    ...
    await renderSite({ cwd, config: defaultConfig, graph })
    const html = await readFile(join(cwd, '.docfuse/dist/markdown/playground/index.html'), 'utf8')

    expect(html).toContain('<html lang="zh" class="df-playground-page">')
    expect(html).toContain('class="df-shell df-shell-playground"')
    expect(html).toContain('data-docfuse-playground=""')
    expect(html).toContain('data-view="preview"')
    expect(html).toContain('data-docfuse-playground-source=""')
    expect(html).toContain('# Playground\n\nUse **one source** for both panes.</textarea>')
    expect(html).toContain('<strong data-df-element="strong">one source</strong>')
    expect(html).toContain('data-docfuse-playground-toggle=""')
    expect(html).toContain('data-docfuse-playground-resizer=""')
    expect(html).toContain('data-docfuse-playground-preview=""')
    expect(html).toContain('data-docfuse-playground-client-url="/assets/docfuse-playground/index.js"')
    expect(html).toContain('src="/assets/docfuse-playground/index.js"')
    expect(html).toContain('role="separator"')
    expect(html).not.toContain('class="df-playground-pane-head"')
    expect(html).not.toContain('class="df-page-footer"')
    expect(html).not.toContain('class="df-outline"')
    await access(join(cwd, '.docfuse/dist/assets/docfuse-playground/index.js'))
  })

  it('rejects public files that overwrite generated runtime assets', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-public-collision-'))
    await mkdir(join(cwd, 'docs/public/assets'), { recursive: true })
    await writeFile(join(cwd, 'docs/public/assets/docfuse.css'), 'malicious overwrite')
    const graph = createMockGraph()

    await expect(renderSite({ cwd, config: defaultConfig, graph })).rejects.toThrow(
      'Static asset conflicts with generated output: assets/docfuse.css'
    )
  })

  it('rejects file and directory overlaps within the generated output plan before writing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-generated-collision-'))
    const page = createMockPage({
      sourcePath: join(cwd, 'docs/zh/assets/docfuse.css.md'),
      transformedSource: '# Collision',
      sourceRelativePath: 'docs/zh/assets/docfuse.css.md',
      relativePath: 'zh/assets/docfuse.css.md',
      version: 'current',
      versionBase: '/',
      docsDir: 'docs',
      locale: 'zh',
      routePath: '/assets/docfuse.css/',
      outputPath: 'assets/docfuse.css/index.html',
      markdownOutputPath: 'assets/docfuse.css/index.md',
      title: 'Collision',
      description: '',
      order: 0,
      group: '',
    ...
    await expect(access(join(cwd, '.docfuse/dist/assets/docfuse.css'))).rejects.toMatchObject({
      code: 'ENOENT'
    })
  })

  it('rejects case-insensitive public collisions for portable output', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-public-case-collision-'))
    await mkdir(join(cwd, 'docs/public/ASSETS'), { recursive: true })
    await writeFile(join(cwd, 'docs/public/ASSETS/DOCFUSE.CSS'), 'portable collision')
    const graph = createMockGraph()
  ...
  it('rejects public files that would be overwritten by generated metadata', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-public-metadata-collision-'))
  ...
  it('publishes authored assets and can omit the header for embedded sites', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-assets-'))
    await mkdir(join(cwd, 'docs/public'), { recursive: true })
    ...
    expect(html).toContain('<html lang="zh" class="df-header-hidden">')
    ...
    expect(html).toContain('class="df-file-name">guide.pdf</span>')
    await access(join(cwd, '.docfuse/dist/logo.svg'))
    await access(join(cwd, '.docfuse/dist/guide/images/local.png'))
    await access(join(cwd, '.docfuse/dist/guide/guide.pdf'))
  })

  it('rejects relative page assets reached through symbolic links', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-page-asset-symlink-'))
  ...
  it('publishes public assets from every configured documentation version', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-versioned-public-'))
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/current.txt'), 'utf8')).toBe('current')
    expect(await readFile(join(cwd, '.docfuse/dist/legacy.txt'), 'utf8')).toBe('legacy')
  })

  it('deduplicates identical public assets shared by documentation versions', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-versioned-public-shared-'))
    await mkdir(join(cwd, 'docs/current/public'), { recursive: true })
    await mkdir(join(cwd, 'docs/v1/public'), { recursive: true })
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/logo.svg'), 'utf8')).toBe('<svg/>')
  })

  it('writes static HTML with page body content and markdown source', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-render-'))
    const outputDir = '.docfuse/dist'
    ...
    expect(html).toContain('docfuse-search.js')
    expect(html).toContain('href="/project/assets/docfuse.css"')
    expect(html).toContain('class="df-brand-logo df-brand-logo-light"')
    expect(html).toContain('src="/project/logo.svg"')
    expect(html).toContain('class="df-brand-logo df-brand-logo-dark"')
    expect(html).toContain('src="/project/logo-dark.svg"')
    expect(html).toContain('src="/project/assets/docfuse-search.js"')
    expect(html).toContain('data-search-index-url="/project/search/zh.json"')
    expect(html).not.toContain('df-markdown-menu')
    expect(html).not.toContain('/markdown/examples/')
    expect(html).toContain('data-empty-label="没有找到匹配文档"')
    expect(html).toContain('data-docfuse-action-success="源文档已复制"')
    expect(html).toContain('data-docfuse-search-default')
    expect(html).not.toContain('data-docfuse-search-action="theme"')
    expect(html).not.toContain('class="df-menu df-lang"')
    expect(html).toContain('class="df-outline-ad"')
    expect(html).toContain('class="df-sidebar-primary-nav"')
    ...
    // Mermaid is owned by @docfuse/plugins. Core keeps an unconfigured fence readable
    // instead of silently activating plugin-specific runtime behavior.
    expect(html).not.toContain('data-df-mermaid-module-url')
    expect(html).not.toContain('df-mermaid-fallback')
    expect(html).toContain('flowchart LR')
    await access(join(cwd, outputDir, 'guide/index.md'))
    await access(join(cwd, outputDir, 'assets/docfuse.css'))
    await expect(access(join(cwd, outputDir, 'assets/fonts'))).rejects.toMatchObject({ code: 'ENOENT' })
    const css = await readFile(join(cwd, outputDir, 'assets/docfuse.css'), 'utf8')
    expect(css).toContain('/* User style: ./docs/brand.css */')
    expect(css).toContain('.brand-test { color: rebeccapurple; }')
    expect(css.lastIndexOf('.brand-test')).toBeGreaterThan(css.lastIndexOf('/* Docfuse shell */'))
    expect(css).toContain('--df-accent-500: #0088ff')
    expect(css).toContain('--df-body-font-size: 1rem')
    expect(css).not.toContain('--df-material-')
    expect(css).not.toMatch(/Nocturne|\.df-bento-grid|\.df-hero-bento/i)
    expect(css).toContain('--df-glass-blur: 18px')
    expect(css).toContain('padding:1.75rem var(--df-site-gutter) 3.5rem')
    const searchClient = await readFile(join(cwd, outputDir, 'assets/docfuse-search.js'), 'utf8')
    expect(searchClient).toContain('data-docfuse-search-default')
    expect(searchClient).toContain('data-docfuse-search-action')
    const notFound = await readFile(join(cwd, outputDir, '404.html'), 'utf8')
    expect(notFound).toContain('<html lang="zh">')
    expect(notFound).toContain('<h1 id="docfuse-404-title" class="df-not-found-title">页面未找到</h1>')
    expect(notFound).toContain('<link rel="icon" href="/project/favicon.svg">')
    expect(notFound).toContain('href="/project/assets/docfuse.css"')
    expect(notFound).toContain('href="/project/"')
    const islandRoot = join(cwd, outputDir, 'assets/docfuse-markdown')
    ...
    const staticOutputDir = '.docfuse/static'
    ...
    await expect(access(join(cwd, staticOutputDir, 'assets/docfuse-markdown'))).rejects.toMatchObject({
      code: 'ENOENT'
    })

    const mathOutputDir = '.docfuse/math'
    const mathPage = {
      ...basePage,
      routePath: '/formula/',
    ...
    const mathCss = await readFile(join(cwd, mathOutputDir, 'assets/docfuse.css'), 'utf8')
  ...
  it('renders a configured third locale across navigation, language menu, and 404', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-render-locale-'))
    const outputDir = '.docfuse/ja'
    await mkdir(join(cwd, 'docs'), { recursive: true })
    const page = createMockPage({
    ...
    expect(html).toContain('class="df-home" tabindex="-1" data-pagefind-body=""')
    expect(html).toContain('<header class="df-header df-header-home">')
    expect(html).not.toContain('class="df-progress"')
