...
import type { DocfuseExtensionDescriptor } from '../config/types'
...
function extensionPage(page: DocPage): DocfuseExtensionPage {
  return {
...
function assertDefinition(value: unknown, descriptor: DocfuseExtensionDescriptor): DocfuseExtension {
  if (!value || typeof value !== 'object') {
    throw new Error(`Extension ${descriptor.resolve} must export an extension object or factory`)
  }
  const definition = value as Partial<DocfuseExtension> & Record<string, unknown>
  ...
  if (definition.apiVersion !== DOCFUSE_EXTENSION_API_VERSION) {
    throw new Error(
      `Extension ${descriptor.resolve} uses unsupported apiVersion ${String(definition.apiVersion)}; expected ${DOCFUSE_EXTENSION_API_VERSION}`
    )
  }
  ...
  return definition as DocfuseExtension
}

function assertPagePatch(name: string, patch: unknown): DocfuseExtensionPagePatch {
  ...
  return patch as DocfuseExtensionPagePatch
}
...
async function loadOneExtension(
  ...
  descriptor: DocfuseExtensionDescriptor
  ...
  const result = await build({
    ...
    plugins: [
      {
        name: 'docfuse-extension-api',
        setup(pluginBuild) {
          pluginBuild.onResolve({ filter: /^docfuse$/ }, () => ({
            path: 'docfuse-extension-api',
            namespace: 'docfuse-extension-api'
          }))
          pluginBuild.onLoad({ filter: /.*/, namespace: 'docfuse-extension-api' }, () => ({
            loader: 'js',
            contents: `
              export const DOCFUSE_EXTENSION_API_VERSION = ${JSON.stringify(DOCFUSE_EXTENSION_API_VERSION)}
              export const docfuseVersion = ${JSON.stringify(docfuseVersion)}
  ...
  try {
    ...
    const value =
      typeof exported === 'function'
        ? await (exported as DocfuseExtensionFactory)(structuredClone(descriptor.options ?? {}))
        : exported
...
export interface ExtensionHost {
  ...
  transformSource(context: DocfuseExtensionSourceContext): Promise<string>
...
export async function loadExtensionHost(
  cwd: string,
  temporaryRoot: string,
  descriptors: readonly DocfuseExtensionDescriptor[]
