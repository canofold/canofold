...
export const RESERVED_OUTPUT_DIRECTORIES = [
  'assets/docfuse-markdown',
  'assets/docfuse-playground',
  'assets/docfuse-plugins',
  'assets/fonts',
  'pagefind',
  'ai/content',
  'extensions'
...
function plannedOutputPaths(config: DocfuseConfig, graph: ContentGraph) {
  const paths = [
    '.benchmark.json',
    '404.html',
    'assets/docfuse.css',
  ...
  if (config.search.enabled) {
    paths.push('assets/docfuse-search.js')
...
export function generatedOutputPaths(config: DocfuseConfig, graph: ContentGraph) {
  const { paths } = plannedOutputPaths(config, graph)
  assertGeneratedOutputPlan(paths)
  return new Set(paths)
}

export function generatedPublicPaths(config: DocfuseConfig, graph: ContentGraph) {
