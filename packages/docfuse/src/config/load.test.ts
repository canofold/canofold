...
describe('loadConfig', () => {
  it('returns defaults when docfuse.config.ts is absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    const config = await loadConfig(cwd)

    expect(config.title).toBe('Docfuse')
    expect(config.docsDir).toBe('docs')
    expect(config.outputDir).toBe('.docfuse/dist')
  ...
  it('can disable the site header for embedded documentation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { layout: { header: false } }`)
  ...
  it('loads docfuse.config.ts and merges defaults', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
  ...
  it('loads relative imports from docfuse.config.ts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    ...
    await expect(readdir(join(cwd, '.docfuse/tmp'))).resolves.toEqual([])
    await expect(access(join(cwd, '.docfuse.config.mjs'))).rejects.toMatchObject({ code: 'ENOENT' })
  })

  it('removes temporary config directories when bundling fails', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-error-'))
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { broken: }`)
  ...
  it('preserves package-relative runtime resolution for official plugins', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    const pluginRoot = join(cwd, 'node_modules/@docfuse/plugins')
    ...
    await writeFile(
      join(cwd, 'docfuse.config.ts'),
      `import { title } from '@docfuse/plugins'; export default { title }`
    )
  ...
  it.each(['cts', 'cjs'])('loads a CommonJS %s configuration entry', async (extension) => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(join(cwd, `docfuse.config.${extension}`), `module.exports = { title: 'CommonJS config' }`)
  ...
  it('rejects ambiguous multiple configuration files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default {}`)
    await writeFile(join(cwd, 'docfuse.config.mjs'), `export default {}`)
    await expect(loadConfig(cwd)).rejects.toThrow('Multiple Docfuse configuration files found')
  })
  ...
  it('rejects extension package specifiers and non-JSON options', async () => {
    const packageSpecifier = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(packageSpecifier, 'docfuse.config.ts'),
    ...
    const nonJson = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(nonJson, 'docfuse.config.ts'),
  ...
  it('rejects an invalid default locale and duplicate locale entries', async () => {
    const missingDefault = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(missingDefault, 'docfuse.config.ts'),
    ...
    const duplicates = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(duplicates, 'docfuse.config.ts'),
  ...
  it('rejects invalid and unregistered locale resource keys', async () => {
    const invalidLocale = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(invalidLocale, 'docfuse.config.ts'),
    ...
    const unknownResource = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(unknownResource, 'docfuse.config.ts'),
    ...
    const misspelledGroup = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(misspelledGroup, 'docfuse.config.ts'),
      `export default {
        i18n: {
          locales: ['zh'],
  ...
  it('loads the Markdown HTML policy', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { markdown: { html: 'sanitize' } }`)
  ...
  it('validates bounded AI artifact settings', async () => {
    ...
    const invalid = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(invalid, 'docfuse.config.ts'),
  ...
  it('rejects accepted-looking settings that would have no effect', async () => {
    const versioned = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(versioned, 'docfuse.config.ts'),
      `export default {
        docsDir: 'docs',
    ...
    const darkLogo = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(darkLogo, 'docfuse.config.ts'),
    ...
    const missingLogo = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(
      join(missingLogo, 'docfuse.config.ts'),
  ...
  it('keeps an overridden docsDir as the implicit current version root', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { docsDir: 'handbook' }`)
    const config = await loadConfig(cwd)
  ...
  it('loads a safe deployment base path and rejects traversal', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-'))
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { basePath: '/project/' }`)
    await expect(loadConfig(cwd)).resolves.toMatchObject({ basePath: '/project/' })

    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { basePath: '/../../escape/' }`)
    await expect(loadConfig(cwd)).rejects.toThrow()
  })

  it('canonicalizes Unicode routes and rejects ambiguous repeated slashes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-route-'))
    await writeFile(
      join(cwd, 'docfuse.config.ts'),
      `export default {
        basePath: '/文档/',
    ...
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { basePath: '//docs/' }`)
    await expect(loadConfig(cwd)).rejects.toThrow('safe absolute path')
  })

  it('requires siteUrl to be an origin without a deployment path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-origin-'))
    await writeFile(join(cwd, 'docfuse.config.ts'), `export default { siteUrl: 'https://example.com/docs/' }`)
  ...
  it('rejects unsafe and portable-duplicate version ids', async () => {
    for (const ids of [
      ...
      const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-version-id-'))
      await writeFile(
        join(cwd, 'docfuse.config.ts'),
  ...
  it('rejects protocol-relative URLs in public resource fields', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-config-resource-'))
    await writeFile(
      join(cwd, 'docfuse.config.ts'),
