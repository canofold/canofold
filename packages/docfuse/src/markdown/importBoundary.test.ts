...
describe('prepareMdxSource', () => {
  ...
  it('rejects retired package component imports', () => {
    expect(() => prepareMdxSource(`import { Alert } from 'docfuse/components'`)).toThrow(
      'External import is not allowed: docfuse/components'
    )
    expect(() => prepareMdxSource(`import { MarkdownBadge } from '@docfuse/markdown'`)).toThrow(
      'External import is not allowed: @docfuse/markdown'
    )
  })
