...
describe('SEO writers', () => {
  ...
  it('uses siteUrl for sitemap and robots output', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'docfuse-seo-'))
    ...
    expect(await readFile(join(cwd, '.docfuse/dist/sitemap.xml'), 'utf8')).toContain(
      'https://docs.example.com/'
    )
    expect(await readFile(join(cwd, '.docfuse/dist/robots.txt'), 'utf8')).toContain(
  ...
  it('does not emit invalid relative sitemap URLs when siteUrl is absent', async () => {
    ...
    await expect(access(join(cwd, '.docfuse/dist/sitemap.xml'))).rejects.toMatchObject({ code: 'ENOENT' })
    expect(await readFile(join(cwd, '.docfuse/dist/robots.txt'), 'utf8')).not.toContain('Sitemap:')
  })
  ...
  it('escapes sitemap XML values', async () => {
    ...
    await writeSitemap(cwd, config, graph)
    const sitemap = await readFile(join(cwd, '.docfuse/dist/sitemap.xml'), 'utf8')
