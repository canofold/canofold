...
function versionedNavLink(link: string, version: CanofoldVersionItem) {
  if (!link.startsWith('/') || version.base === '/') return link
  return `${version.base}${link.replace(/^\//, '')}`
}

function buildConfiguredNav(items: CanofoldNavigationItem[], version: CanofoldVersionItem): NavItem[] {
