...
export function resolveBuildCacheRoot(cwd: string) {
  return resolveProjectPath(cwd, '.canofold/cache', 'build cache')
}

export function resolveBuildTemporaryRoot(cwd: string) {
  return resolveProjectPath(cwd, '.canofold/tmp', 'temporary build files')
}
...
function isBuildManifest(value: unknown): value is BuildManifest {
  ...
  return (
    ...
    manifest.canofoldVersion === canofoldVersion &&
...
async function acquireBuildLock(path: string) {
  try {
    ...
    if (!(await removeStaleLock(path))) {
      throw new Error('Another Canofold build is already running for this project')
    }
    try {
      return await createBuildLock(path)
    } catch (retryError) {
...
async function assertBuildLockOwned(lock: BuildLock) {
  ...
  if (current?.ownerId !== lock.ownerId) {
    throw new Error('Canofold build lock ownership was lost during the build')
  }
}
...
export async function withBuildLock<T>(
  ...
  const refreshTimer = setInterval(() => {
    void refreshBuildLock(lock)
      .then((owned) => {
        if (!owned) refreshError = new Error('Canofold build lock ownership was lost during the build')
      })
      .catch((error: unknown) => {
        refreshError = error
      })
