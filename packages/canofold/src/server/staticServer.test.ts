...
describe('resolveRequestPath', () => {
  ...
  it('injects live reload script for dev HTML responses', () => {
    ...
    expect(html).toContain('/__canofold/events')
  ...
  it('does not follow symlinks outside the static root', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-static-'))
  ...
  it('serves GET and HEAD requests with base paths, live reload, and generated errors', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-static-server-'))
    ...
    try {
      ...
      expect(home.body).toContain('/docs/__canofold/events')
      ...
      expect(missing.body).toContain('/docs/__canofold/events')
  ...
  it('reports an unsafe generated 404 as a server error instead of hiding it', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-static-error-'))
    const root = join(cwd, 'site')
    await mkdir(root)
  ...
  it('streams page updates and build status over the mounted live-reload endpoint', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-static-events-'))
    await writeFile(join(cwd, 'index.html'), '<html><body>Home</body></html>')
    let currentRoot = cwd
    ...
    try {
      stream = await connectReloadStream(server.port, '/docs/__canofold/events')
