...
describe('writeSearchIndexes', () => {
  it('keeps compact built in and explains how to migrate the legacy Pagefind option', () => {
    ...
    expect(() => searchProviderFor('pagefind')).toThrow('@canofold/plugins')
  })
  ...
  it('writes compact documents and a locale-scoped inverted index', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-search-'))
    ...
    const index = JSON.parse(await readFile(join(cwd, '.canofold/dist/search/zh.json'), 'utf8'))
  ...
  it('indexes the complete searchable body without serializing it twice', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-search-large-'))
    await mkdir(join(cwd, '.canofold/dist'), { recursive: true })
  ...
  it('does not emit a search index when search is disabled', async () => {
    ...
    await expect(readFile(join(cwd, '.canofold/dist/search/zh.json'), 'utf8')).rejects.toMatchObject({
      code: 'ENOENT'
    })
  })

  it('runs a custom search provider with stable public and filesystem helpers', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-search-provider-'))
    await mkdir(join(cwd, '.canofold/dist'), { recursive: true })
    ...
    await expect(readFile(join(cwd, '.canofold/dist/custom-index.json'), 'utf8')).resolves.toBe(
      '{"url":"/guide/"}'
    )
  })
})
