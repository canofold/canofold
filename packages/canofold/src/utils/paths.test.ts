...
describe('resolveOutputRoot', () => {
  it('resolves an output directory inside the project root', () => {
    expect(resolveOutputRoot('/tmp/project', '.canofold/dist')).toBe(join('/tmp/project', '.canofold/dist'))
  })
...
describe('output path safety', () => {
  it('rejects project paths that escape through a symbolic link', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-project-path-'))
    const outside = await mkdtemp(join(tmpdir(), 'canofold-project-path-outside-'))
  ...
  it('rejects overlapping input and output directories', async () => {
    await expect(
      assertDisjointPaths('/tmp/project/.canofold/dist', '/tmp/project/docs', 'docsDir')
  ...
  it('rejects overlap hidden behind a symbolic link', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-path-safety-'))
