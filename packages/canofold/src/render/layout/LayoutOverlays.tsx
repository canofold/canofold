...
function QuickAction({
  ...
  const content = (
    <>
      <span className="cf-command-glyph" aria-hidden="true">
        {icon}
      </span>
      <span>
        <strong>{title}</strong>
  ...
  return href ? (
    <a className="cf-command-item" href={href}>
      {content}
    </a>
  ) : (
    <button className="cf-command-item" type="button" data-canofold-search-action={action}>
      {content}
    </button>
  )
}
...
function SearchModal({ model }: { model: LayoutModel }) {
  ...
  return (
    <div
      className="cf-search-modal"
      data-canofold-search=""
      ...
      hidden
    >
      <div className="cf-search-backdrop" data-canofold-search-close="" aria-hidden="true" />
      <div
      ...
      >
        <div className="cf-search-field">
          ...
          />
          <button
            className="cf-search-close"
            type="button"
            data-canofold-search-close=""
            aria-label={labels.close}
          >
            Esc
          </button>
        </div>
        <div className="cf-search-body">
          <div className="cf-search-default" data-canofold-search-default="">
            <p className="cf-search-section-label">{labels.quickActions}</p>
            {hasMarkdownShowcase ? (
              <>
                <QuickAction
          ...
          <div className="cf-search-results" data-canofold-search-results="" aria-live="polite" hidden />
        </div>
      </div>
    </div>
  )
...
function SourceSheet({ model }: { model: LayoutModel }) {
  ...
  if (home) return null
  return (
    <div className="cf-source-sheet" data-canofold-source-sheet="" hidden>
      <div className="cf-source-backdrop" data-canofold-source-close="" aria-hidden="true" />
      <aside
        className="cf-source-panel"
        role="dialog"
        aria-modal="true"
        aria-label={labels.source}
        tabIndex={-1}
      >
        <div className="cf-source-head">
          <div>
            <p>{labels.source}</p>
            <span>{page.sourceRelativePath}</span>
          </div>
          <div className="cf-source-actions">
            <button
              className="cf-icon-button"
              type="button"
              data-canofold-source-copy=""
              data-canofold-action-success={labels.sourceCopied}
