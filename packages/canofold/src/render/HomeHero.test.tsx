...
describe('HomeHero', () => {
  it('renders configured landing content with deployment-aware links', () => {
      ...
      frontmatter: {
        hero: {
          ...
          actions: [
            ...
            { text: 'GitHub', link: 'https://github.com/example/canofold' }
          ]
        },
        features: [
          {
    ...
    expect(html).toContain('<h1 class="cf-hero-title">Build documentation</h1>')
    ...
    expect(html).toContain('href="https://github.com/example/canofold"')
    expect(html).toContain('Static output')
    expect(html).toContain('Deploy anywhere.')
    expect(html).toContain('class="cf-hero cf-hero-with-visual"')
    expect(html).toContain('<article class="cf-feature">')
    expect(html).not.toContain('cf-feature-footer')
  ...
  it('omits the visual column when the home page has no image', () => {
    ...
    expect(html).toContain('class="cf-hero"')
    expect(html).not.toContain('cf-hero-with-visual')
  ...
  it('uses the page title as the single source of truth for the home heading', () => {
    ...
    expect(html).toContain('<h1 class="cf-hero-title">Documentation</h1>')
    expect(html).toContain('<p class="cf-hero-accent">Built once</p>')
  })
  ...
  it('does not render an empty icon container for text-only features', () => {
    ...
    expect(html).toContain('Static output')
    expect(html).not.toContain('cf-feature-icon')
  })
})
