...
export async function copyMarkdownClient(outputRoot: string) {
  const entry = require.resolve('@canofold/markdown/client')
  const target = join(outputRoot, 'assets/canofold-markdown')
...
export async function collectPublicAssets(cwd: string, config: CanofoldConfig): Promise<AssetCopy[]> {
