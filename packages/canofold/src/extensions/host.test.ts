...
async function fixture(extensionSource: string) {
  const cwd = await mkdtemp(join(tmpdir(), 'canofold-extension-'))
  ...
  return {
    cwd,
    descriptors,
    temporaryRoot: join(cwd, '.canofold/cache')
  }
}
...
describe('ExtensionHost', () => {
  it('creates a stable no-op host when no extensions are configured', async () => {
    ...
    await expect(host.generate(join(cwd, '.canofold/dist'), graph.pages)).resolves.toBeUndefined()
  })

  it('resolves the public canofold extension API from the bundled host shim', async () => {
    const { cwd, descriptors, temporaryRoot } = await fixture(`
      import { CANOFOLD_EXTENSION_API_VERSION, defineExtension, canofoldVersion } from 'canofold'
      export default defineExtension({
        apiVersion: CANOFOLD_EXTENSION_API_VERSION,
        name: 'public-api',
        extendPage(page) { return { description: 'Canofold ' + canofoldVersion + ': ' + page.title } }
      })`)
    const host = await loadExtensionHost(cwd, temporaryRoot, descriptors)
    const graph = await buildContentGraph(cwd, defaultConfig, host)

    expect(graph.pages[0]?.description).toMatch(/^Canofold \d+\.\d+\.\d+: Home$/)
  })
  ...
  it('rejects incompatible APIs, duplicate identities and undeclared outputs', async () => {
    ...
    await expect(host.generate(join(undeclared.cwd, '.canofold/dist'), [])).rejects.toThrow(
      'Extension "unsafe" generate failed'
    )
    await expect(access(join(undeclared.cwd, '.canofold/dist/extensions/escape.txt'))).rejects.toThrow()
  ...
  it('rejects duplicate and missing generated artifacts', async () => {
    ...
    await expect(duplicateHost.generate(join(duplicate.cwd, '.canofold/dist'), [])).rejects.toThrow(
    ...
    await expect(missingHost.generate(join(missing.cwd, '.canofold/dist'), [])).rejects.toThrow(
