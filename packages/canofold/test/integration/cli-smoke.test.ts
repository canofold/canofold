...
const cliPath = resolve(repoRoot, 'packages/canofold/dist/cli.js')

describe('CLI smoke', () => {
  it('prints help', async () => {
    const result = await execFileAsync('node', [cliPath, '--help'], { cwd: repoRoot })
    expect(result.stdout).toContain('canofold')
  ...
  it('creates a selected single-language project through the published CLI', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-cli-smoke-init-'))
    try {
      await execFileAsync('node', [cliPath, 'init', '--locale', 'en'], { cwd })
      await expect(readFile(join(cwd, 'docs/index.md'), 'utf8')).resolves.toContain('# Welcome to Canofold')
      await expect(readFile(join(cwd, 'canofold.config.ts'), 'utf8')).resolves.toContain("locales: ['en']")
    } finally {
  ...
  it('does not guess the language of existing documents in a non-interactive process', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'canofold-cli-smoke-adopt-'))
    try {
