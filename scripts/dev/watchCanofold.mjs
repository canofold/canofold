...
const packageRoot = join(repoRoot, 'packages/canofold')
const statePath = join(repoRoot, '.canofold-dev/canofold.json')
...
const controller = await startPackageBuildWatcher({
  ...
  onError: (error) => console.error('[canofold:canofold]', error)
})
